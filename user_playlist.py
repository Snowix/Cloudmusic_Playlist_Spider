...
def source_parser(user, page):
    c_num = driver.find_element_by_css_selector('#cHeader > h3 > span')
    num = int(re.search(r'\d+',c_num.text))
    ...
    save_playlist(user, driver, title_list, play_list)

def save_playlist(user, driver, title_list, play_list):
